<?php $this->load->view('layout/header');?>
    <article>
    <img src="<?= base_url(); ?>assets/front/images/terms.jpg" width="100%">

    <div class="container">

   
    <div class="row">
        <div class="col-md-6 col-md-offset-3">
        <h2 class="text-danger" style="text-align:center;margin-bottom:30px;"><b> Certificate of Incorporation</b></h2>
        <img src="<?= base_url(); ?>assets/front/images/certificate.png" alt="" width="100%">
        </div>
    </div>
    <div class="row">
        <div class="col-md-6">
        <h2 class="text-danger" style="text-align:center;margin-bottom:30px;"><b> Tax Deduction Account Number</b></h2>
        <img src="<?= base_url(); ?>assets/front/images/tan.png" alt="" width="100%">
        </div>
   
        <div class="col-md-6">
        <h2 class="text-danger" style="text-align:center;margin-bottom:30px;"><b> Permanent Account Number</b></h2>
        <img src="<?= base_url(); ?>assets/front/images/pan.png" alt="" width="100%">
        </div>
    </div>
    </div>

<div class="container">



</div>
    <section class="relative fix" id="App" style="margin-top:25px">
        <!-- <div class="space-80"></div> -->
        <div class="section-bg overlay-bg">
            <img src="<?= base_url(); ?>assets/front/images/a_bg.jpg" alt="">
        </div>
        <div class="container">
            <div class="row wow fadeInUp">
            <div class="col-xs-12 col-sm-3   wow fadeInRight ">
                    <img src="<?= base_url(); ?>assets/front/images/mobile.png" alt="" width="100%">
                </div>
                <div class="col-xs-12 col-sm-9 text-center text-white">
                <div class="space-60"></div>
                    <h3 class="text-capitalize">So download the "My Information" App today</h3>
                    <p>You can also download this "App" by typing My Information on Google Play Store or clicking on the link given here -</p>
                        <!-- <div class="space-40"></div> -->
                        <a href="#" class="big-button aligncenter">
                        <span class="big-button-icon">
                            <span class="fa fa-android"></span>
                        </span>
                        <span>Available on</span>
                        <br>
                        <strong>Play Store</strong>
                    </a>
                    <p style="margin-top:20px">आप Google Play Store पर My Information लिखकर या यहाँ दिए गए लिंक पर क्लिक करके भी इस "App" को डाउनलोड कर सकते हैं।</p>
                </div>
                
            </div>
            <!-- <div class="space-60"></div> -->
           
        </div>
        <!-- <div class="space-80"></div> -->
    </section>
    <!--Download-Section/-->

    <!--Question-section-->
    
    <br>
    <br>
    </article>   
<?php $this->load->view('layout/footer');?>