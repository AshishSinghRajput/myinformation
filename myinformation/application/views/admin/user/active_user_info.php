    ...
    <div class="row">
    	 <div class="col-sm-12">
            <div class="card-box table-responsive">
                <h4 class="m-t-0 header-title"><b>Activate User's Information</b></h4>
                ...
                <table id="datatable-buttons" class="table table-striped table-bordered table-hover">
                    ...
                    <tr>
						...
						<th>User Current Step Income</th>
						<th>User Current Matching Income</th>
                        <th>User Current Total Income</th>
                        <th>Sponsor's Id</th>
                    ...
                    <tbody>
                        <?php
                        $count = 1; $match=''; $step=''; $tot_income='';
                        if(!empty($user)) {
                          foreach ($user as $row) {
                            $activInfo = $this->welcome_model->getActiveUserInfo($row->sponsor_id);
                            $userTotStep = $this->welcome_model->totalStepIncome($row->sponsor_id);
                            $userTotMatch = $this->welcome_model->totalMatchIncome($row->sponsor_id);
                           
                            if($userTotStep->tot_step!='')
                            {
                                $step = $userTotStep->tot_step; 
                            }else{
                                $step = '0.00';
                            }
                            if($userTotMatch->tot_match!='')
                            {
                                $match = $userTotMatch->tot_match; 
                            }else{
                                $match = '0.00';
                            }
                             $tot_income = $step + $match;
                         ?>
                    <tr> 
                        <td><?= $count++;?></td>
