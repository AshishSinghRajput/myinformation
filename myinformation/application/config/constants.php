...
$user = array('login','signup','tree','bank','profiles','setting','logout','my_information','profile_matching','send_message','all_document','resume','plan','level','upgrade_plan','steps_income','matching_incomes','create_pin','transfer_epin','activation','upgration','about','faq','contact','regional_experts','privacy_policy','terms_condition','expenses','edit_expenses','other_income','edit_other_income','special','legal','share_earn','our_bank','plan_purchase','packege','mymatching_planwise_downlines');

$admin =array('masteradmin','adminprofile','sliders','change','admin_logout','userlist','member_profile','full_profile','step_income','matching_income','epin_history','downline_history','generate_password','latest_news','testmonial','importent','regional','view_slider','religions','add_religions','caste','add_caste','sub_caste','add_sub_caste','free_user','all_epin','user_plan','user_income','payment_history','multiple_id_user','plans','add_plans','upgrade_notice','add_upgrade_notice','income_datewise','downlines_earning','direct_downlines_earning','matching_planwise_downlines','step_planwise_downline','new_step_income','active_user_info','pending_user_info','block_user_info','block','unblock','bank_information','paytm_information','phonepe_information','bhim_information','googlepay_information','account_information','user_total_income_weekly','user_history','user_all_history');  
...
define('upgrade_notice', 'upgrade_notice');
define('PLAN_ACTIVATION_HISTORY', 'plan_activation_history');
...
// defined($user[37]) or define('mymatching_planwise_downlines', BASEURL . 'mymatching_planwi
/**==============================Admin Panel====================================== */
...
defined($admin[32]) or define('upgrade_notice', BASEURL . 'upgrade_notice');
defined($admin[33]) or define('add_upgrade_notice', BASEURL . 'add_upgrade_notice');
defined($admin[34]) or define('income_datewise', BASEURL . 'income_datewise');
defined($admin[35]) or define('downlines_earning', BASEURL . 'downlines_earning');
defined($admin[36]) or define('direct_downlines_earning', BASEURL . 'direct_downlines_earning');
defined($admin[37]) or define('matching_planwise_downlines', BASEURL . 'matching_planwise_downlines');
defined($admin[38]) or define('step_planwise_downline', BASEURL . 'step_planwise_downline');
defined($admin[39]) or define('new_step_income', BASEURL . 'new_step_income');
defined($admin[40]) or define('active_user_info', BASEURL . 'active_user_info');
defined($admin[41]) or define('pending_user_info', BASEURL . 'pending_user_info');
defined($admin[42]) or define('block_user_info', BASEURL . 'block_user_info');
defined($admin[43]) or define('block', BASEURL . 'block');
defined($admin[44]) or define('unblock', BASEURL . 'unblock');
defined($admin[45]) or define('bank_information', BASEURL . 'bank_information');
defined($admin[46]) or define('paytm_information', BASEURL . 'paytm_information');
defined($admin[47]) or define('phonepe_information', BASEURL . 'phonepe_information');
defined($admin[48]) or define('bhim_information', BASEURL . 'bhim_information');
defined($admin[49]) or define('googlepay_information', BASEURL . 'googlepay_information');
defined($admin[50]) or define('account_information', BASEURL . 'account_information');
defined($admin[51]) or define('user_total_income_weekly', BASEURL . 'user_total_income_weekly');
 defined($admin[52]) or define('user_history', BASEURL . 'user_history');
// defined($admin[53]) or define('user_all_history', BASEURL . 'user_all_history');